...
class ZmqPub:
    ...
    def __init__(
        ...
        self._max_reconnect_delay = float(globals().get("TICK_PUBLISHER_MAX_RECONNECT_DELAY", TICK_PUBLISHER_RECONNECT_DELAY * 60))
        self._last_reconnect_attempt = 0.0
        # queueing/backpressure
        self._max_queue_size = int(TICK_PUBLISHER_MAX_QUEUE)
        self._send_queue: "queue.Queue" = queue.Queue(maxsize=self._max_queue_size)
        self._sender_thread: Optional[threading.Thread] = None
        self._sender_shutdown = threading.Event()
        # config: dead-letter file, backpressure threshold, batch size, lag warn
        self._dead_letter_file = TICK_PUBLISHER_DEAD_LETTER_FILE
        self._backpressure_threshold = float(TICK_PUBLISHER_BACKPRESSURE_THRESHOLD)
        self._batch_size = int(TICK_PUBLISHER_BATCH_SIZE)
        self._lag_warn_threshold = float(TICK_PUBLISHER_LAG_WARN_THRESHOLD)
        self._start_sender()
    ...
    def _connect(self) -> bool:
        ...
        with self._lock:
            ...
            try:
                ...
                # increase reconnect attempts to backoff exponentially
                try:
    ...
    def _should_reconnect(self) -> bool:
        ...
        try:
            base = float(self._reconnect_delay)
        except Exception:
            base = 0.5
        try:
            attempts = int(self._reconnect_attempts)
        except Exception:
            attempts = 0
        delay = min(base * (2 ** attempts), self._max_reconnect_delay)
    ...
    def publish(self, topic: str, payload: dict, max_retries: int = 2) -> bool:
        ...
        try:
            ...
            self._send_queue.put_nowait((topic, payload, enqueue_time))
            # backpressure detection
            try:
                qsize = self._send_queue.qsize()
                if qsize >= int(self._max_queue_size * self._backpressure_threshold):
                    self.metrics.high_water_events += 1
                    logger.warning(
                        "Send queue high-water: %d/%d", qsize, self._max_queue_size
                    )
            except Exception:
                pass
            return True
        except queue.Full:
            # Queue full -> count as an error/drop and optionally persist to dead-letter
            self.metrics.message_errors += 1
            ...
            if self._dead_letter_file:
                try:
                    with open(self._dead_letter_file, "a", encoding="utf-8") as fh:
                        fh.write(json.dumps({"topic": topic, "payload": payload, "dropped_at": time.time()}) + "\n")
                except Exception:
    ...
    def _start_sender(self):
        ...
        def loop():
            while not self._sender_shutdown.is_set():
                ...
                try:
                    try:
                        topic, payload, enqueue_time = item
                    except Exception:
                    ...
                    try:
                        now = time.time()
                        latency = now - (enqueue_time or now)
                        if latency > self.metrics.max_queue_latency:
                            self.metrics.max_queue_latency = latency
                        # simple running average
                        if self.metrics.avg_queue_latency == 0.0:
                            self.metrics.avg_queue_latency = latency
                        else:
                            self.metrics.avg_queue_latency = (
                                (self.metrics.avg_queue_latency + latency) / 2.0
                            )
                        if latency >= self._lag_warn_threshold:
                            logger.warning("High queue latency: %.3fs", latency)
                    except Exception:
                        pass
                    ...
                    with self._lock:
                        ...
                        sent = False
                        try:
                            # batch ticks when possible
                            if topic == "tick":
                                batch = [payload]
                                # drain up to batch_size-1 additional tick messages
                                for _ in range(self._batch_size - 1):
                                    try:
                                        ...
                                        break
                                    try:
                                        nxt_topic, nxt_payload, nxt_enqueue = nxt
                                    except Exception:
                                        try:
                                            nxt_topic, nxt_payload = nxt
                                        except Exception:
                                            continue
                                    if nxt_topic == "tick":
                                        batch.append(nxt_payload)
                                    else:
                                        # re-enqueue non-tick item at front by putting it back
                                        try:
                                            self._send_queue.put_nowait((nxt_topic, nxt_payload, time.time()))
                                        except Exception:
                                            # if we can't requeue, write to dead-letter
                                            self.metrics.queue_drops += 1
                                            if self._dead_letter_file:
                                                try:
                                                    with open(self._dead_letter_file, "a", encoding="utf-8") as fh:
                                                        fh.write(json.dumps({"topic": nxt_topic, "payload": nxt_payload, "dropped_at": time.time()}) + "\n")
                                                except Exception:
                                                    pass

                                # send batch as a single multipart message
                                if self._socket and self._state == ConnectionState.CONNECTED:
                                    topic_frame = (self.topic_prefix + "tick_batch").encode("utf-8")
                                    json_frame = json.dumps({"type": "system_tick_batch", "items": batch}).encode("utf-8")
                                    self._socket.send_multipart([topic_frame, json_frame])
                                    self.metrics.messages_sent += 1
                                    self.metrics.last_success_time = time.time()
                                    sent = True
                        ...
                        if not sent:
                            ...
                            if self._dead_letter_file:
                                try:
                                    with open(self._dead_letter_file, "a", encoding="utf-8") as fh:
                                        fh.write(json.dumps({"topic": topic, "payload": payload, "failed_at": time.time()}) + "\n")
                                except Exception:
                                    logger.exception("Failed to write dead-letter entry on send failure")
                finally:
                    try:
