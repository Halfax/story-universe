...
def list_inventory(
    owner_type: str,
    owner_id: str,
    db_conn_getter: Optional[Callable[[], sqlite3.Connection]] = None,
) -> List[Dict[str, Any]]:
    conn = _get_conn(db_conn_getter)
    conn.row_factory = sqlite3.Row
    c = conn.cursor()
    c.execute(
        "SELECT i.*, it.name as item_name, it.stackable, it.max_stack, it.equippable, it.equip_slot as item_equip_slot, it.consumable FROM inventory i LEFT JOIN items it ON i.item_id = it.id WHERE owner_type=? AND owner_id=?",
        (owner_type, owner_id),
    )
    ...
    try:
        ...
        with conn:
            if stackable:
                ...
                c.execute(
                    "SELECT * FROM inventory WHERE owner_type=? AND owner_id=? AND item_id=? AND equipped=0",
            ...
            c.execute(
                "INSERT INTO inventory (owner_type, owner_id, item_id, quantity, durability, charges_remaining, equipped, equip_slot, metadata) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    owner_type,
                    owner_id,
                    item_id,
...
def use_inventory_item(
    ...
    c = conn.cursor()
    c.execute(
        "SELECT i.*, it.consumable, it.effects FROM inventory i JOIN items it ON i.item_id = it.id WHERE i.id=?",
        (inventory_id,),
    )
    row = c.fetchone()
    if not row:
...
def equip_inventory_item(
    ...
    c = conn.cursor()
    c.execute(
        "SELECT i.*, it.equippable, it.equip_slot as item_equip_slot FROM inventory i JOIN items it ON i.item_id = it.id WHERE i.id=?",
        (inventory_id,),
    )
    row = c.fetchone()
    if not row:
    ...
    try:
        with conn:
            ...
            c.execute(
                "UPDATE inventory SET equipped=0, equip_slot=NULL WHERE owner_type=? AND owner_id=? AND equip_slot=?",
                (owner_type, owner_id, equip_slot),
            )
            # Equip this item
            c.execute(
                "UPDATE inventory SET equipped=1, equip_slot=? WHERE id=?",
