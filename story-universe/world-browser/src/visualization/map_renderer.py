import math
from PySide6.QtWidgets import QGraphicsView, QGraphicsScene, QGraphicsItem, QMenu, QGraphicsSceneMouseEvent
...
class MapRenderer:
    ...
    def add_location(self, name: str, pos: Tuple[float, float], **kwargs):
        """Add a location to the map."""
        if name in self.locations:
            return
            
        item = MapItem(name, pos, **kwargs)
        self.locations[name] = item
        self.graph.add_node(name, pos=pos, item=item)
    ...
    def _update_connections(self):
        ...
        for u, v, data in self.graph.edges(data=True):
            item1 = self.locations[u]
            item2 = self.locations[v]
