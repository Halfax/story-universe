...
def startup_tasks():
    logger.info(
        "[ChronicleKeeper] FastAPI startup event: checking environment and ensuring test DB tables..."
    )
...
def shutdown_tasks():
    logger.info("[ChronicleKeeper] FastAPI shutdown event: scheduling cleanup...")
    import threading

    def _run_with_timeout(func, timeout=2.0):
        t = threading.Thread(target=func)
        t.daemon = True
        t.start()
        t.join(timeout)

    # Stop the world clock (if any) but don't block indefinitely
    try:

        def _stop_clock():
            try:
                from src.services.clock import stop_world_clock

                stop_world_clock()
            except Exception:
                pass

        _run_with_timeout(_stop_clock, timeout=6.0)
    except Exception:
        pass

    # Close module-level publisher with timeout
    try:
        _run_with_timeout(lambda: publisher.close(), timeout=4.0)
    except Exception:
        pass

    # Allow event handlers or other modules to perform cleanup if they expose it
    try:
        if hasattr(event_handlers, "shutdown"):
            _run_with_timeout(lambda: event_handlers.shutdown(), timeout=2.0)
    except Exception:
        pass
    # brief pause to allow OS file locks to be released on Windows
    try:
        import time as _tm
...
if __name__ == "__main__":
    logger.info("[ChronicleKeeper] __main__ entry: starting world clock thread...")
    from src.services.clock import start_world_clock
...
publisher = TickPublisher(
    address=ZMQ_PUB_CLIENT_ADDR, bind=False
)  # Connect, do not bind (address from config)


@app.post("/event")
async def ingest_event(
    event: dict, background: BackgroundTasks, api_key: str = require_api_key()
):
    ...
    conn = get_connection()
    c = conn.cursor()
    # Ensure canonical event id is recorded in metadata so consumers/replay can reference it
    metadata = dict(evd.get("metadata", {}) or {})
    metadata.setdefault("_event_id", evd.get("id"))
    c.execute(
        """
    ...
    try:
        ...
        logger.exception("Error applying event consequences")
    try:
        conn.commit()
    except Exception:
        pass
    ...
    try:
        ...
        # Non-fatal: log and continue
        logger.warning("[ChronicleKeeper] Warning: failed to publish event")
...
def create_character(payload: dict, admin: bool = Depends(require_admin)):
    ...
    c = conn.cursor()
    c.execute(
        "INSERT INTO characters (name, age, traits, location_id, status) VALUES (?, ?, ?, ?, ?)",
        (
            payload.get("name"),
            payload.get("age"),
            str(payload.get("traits", {})),
...
def update_character(char_id: int, payload: dict, admin: bool = Depends(require_admin)):
    ...
    c = conn.cursor()
    c.execute(
        "UPDATE characters SET name=?, age=?, traits=?, location_id=?, status=? WHERE id=?",
        (
            payload.get("name"),
            payload.get("age"),
            str(payload.get("traits", {})),
...
def create_location(payload: dict, admin: bool = Depends(require_admin)):
    ...
    c = conn.cursor()
    c.execute(
        "INSERT INTO locations (name, description, region, forbidden, locked, political_status, metadata) VALUES (?, ?, ?, ?, ?, ?, ?)",
        (
            payload.get("name"),
            payload.get("description"),
            payload.get("region"),
            int(payload.get("forbidden", 0)),
...
def update_location(loc_id: int, payload: dict, admin: bool = Depends(require_admin)):
    ...
    c = conn.cursor()
    c.execute(
        "UPDATE locations SET name=?, description=?, region=?, forbidden=?, locked=?, political_status=?, metadata=? WHERE id=?",
        (
            payload.get("name"),
            payload.get("description"),
            payload.get("region"),
...
def create_faction(payload: dict, admin: bool = Depends(require_admin)):
    ...
    c = conn.cursor()
    c.execute(
        "INSERT INTO factions (name, ideology, relationships) VALUES (?, ?, ?)",
        (
            payload.get("name"),
            payload.get("ideology"),
            str(payload.get("relationships", {})),
        ),
    )
    conn.commit()
    fid = c.lastrowid
...
def update_faction(fid: int, payload: dict, admin: bool = Depends(require_admin)):
    ...
    c = conn.cursor()
    c.execute(
        "UPDATE factions SET name=?, ideology=?, relationships=? WHERE id=?",
        (
            payload.get("name"),
            payload.get("ideology"),
            str(payload.get("relationships", {})),
...
def get_faction_metrics(fid: int):
    ...
    c = conn.cursor()
    c.execute(
        "SELECT trust, power, resources, influence FROM faction_metrics WHERE faction_id = ?",
        (fid,),
    )
    row = c.fetchone()
    conn.close()
    ...
    return {
        "faction_id": fid,
        "trust": float(row[0] or 0.5),
        "power": int(row[1] or 0),
        "resources": int(row[2] or 0),
        "influence": int(row[3] or 0),
    }


@app.put("/world/factions/{fid}/metrics")
def update_faction_metrics(
    fid: int, payload: dict, admin: bool = Depends(require_admin)
):
...
def list_faction_relationships(fid: int):
    ...
    c = conn.cursor()
    c.execute(
        "SELECT id, target_faction_id, relationship_type, strength, last_updated, cooldown_until, metadata FROM faction_relationships WHERE source_faction_id = ?",
        (fid,),
    )
...
def upsert_faction_relationship(
    fid: int, target_id: int, payload: dict, admin: bool = Depends(require_admin)
):
    ...
    c = conn.cursor()
    c.execute(
        "SELECT id FROM faction_relationships WHERE source_faction_id = ? AND target_faction_id = ?",
        (fid, target_id),
    )
    row = c.fetchone()
    if not row:
        c.execute(
            "INSERT INTO faction_relationships (source_faction_id, target_faction_id, relationship_type, strength, last_updated, cooldown_until, metadata) VALUES (?, ?, ?, ?, ?, ?, ?)",
            (
                fid,
                target_id,
                rel_type,
                float(strength or 0.0),
                int(payload.get("last_updated") or 0),
        ...
        params.append(rid)
        c.execute(
            "UPDATE faction_relationships SET " + ", ".join(updates) + " WHERE id = ?",
            params,
        )
    conn.commit()
    conn.close()
...
def list_faction_cooldowns(fid: int):
    ...
    c = conn.cursor()
    c.execute(
        "SELECT id, cooldown_key, until_ts, metadata FROM faction_cooldowns WHERE faction_id = ?",
        (fid,),
    )
    rows = [dict(zip([col[0] for col in c.description], row)) for row in c.fetchall()]
    conn.close()
...
def set_faction_cooldown(
    fid: int, key: str, payload: dict, admin: bool = Depends(require_admin)
):
    ...
    c = conn.cursor()
    c.execute(
        "SELECT id FROM faction_cooldowns WHERE faction_id = ? AND cooldown_key = ?",
        (fid, key),
    )
    row = c.fetchone()
    if not row:
        c.execute(
            "INSERT INTO faction_cooldowns (faction_id, cooldown_key, until_ts, metadata) VALUES (?, ?, ?, ?)",
            (fid, key, int(until_ts), metadata),
        )
        cid = c.lastrowid
    else:
        cid = row[0]
        c.execute(
            "UPDATE faction_cooldowns SET until_ts = ?, metadata = ? WHERE id = ?",
            (int(until_ts), metadata, cid),
        )
    conn.commit()
    conn.close()
...
def get_world_state():
    ...
    try:
        ...
        try:
            ...
            for r in c.fetchall():
                ...
                state["factions"][fid]["metrics"] = {
                    "trust": float(r[1]) if r[1] is not None else 0.5,
                    "power": int(r[2] or 0),
                    "resources": int(r[3] or 0),
                    "influence": int(r[4] or 0),
                }
        except Exception:
            pass
        ...
        try:
            c.execute(
                "SELECT source_faction_id, target_faction_id, relationship_type, strength, cooldown_until FROM faction_relationships"
            )
            for r in c.fetchall():
                src = str(r[0])
                tgt = str(r[1])
                rel = {
                    ...
                    "strength": float(r[3] or 0.0),
                    "cooldown_until": int(r[4] or 0),
                }
        ...
        try:
            c.execute(
                "SELECT faction_id, cooldown_key, until_ts, metadata FROM faction_cooldowns"
            )
            for r in c.fetchall():
                fid = str(r[0])
                state["factions"].setdefault(fid, {})
...
def api_list_inventory(
    owner_type: str, owner_id: str, api_key: str = require_api_key()
):
...
def api_use_inventory(
    inventory_id: int, payload: dict = {}, api_key: str = require_api_key()
):
    quantity = int(payload.get("quantity", 1))
    try:
...
def api_use_inventory_decision(
    inventory_id: int, payload: dict = {}, api_key: str = require_api_key()
):
